...
Discovery tickets live as GitHub issues on this repo (labels `wayfinder:map`, `wayfinder:{research,prototype,grilling,task}`): map = [#1](https://github.com/amit-t/krosval/issues/1), tickets T01–T12 = [#2–#13](https://github.com/amit-t/krosval/issues). To (re)sync from the markdown, run (idempotent — existing issues are matched by title and reused):
